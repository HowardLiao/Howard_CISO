...
def add_heading_1(doc, text, space_before=16, space_after=6, east_asia="Microsoft JhengHei"):
    p = doc.add_paragraph()
...
def add_header(doc, name_text, title_text, subtitle_text, contact_text1, contact_text2, east_asia="Microsoft JhengHei"):
    ...
    col_left.width = Inches(1.8)
    col_right.width = Inches(5.2)
...
def add_exp_item(doc, role, group_note, company_line, period, desc, scopes, achs, leaving=None, add_ctx=None, east_asia="Microsoft JhengHei", labels=None):
    ...
    p_r.paragraph_format.space_before = Pt(14)
    ...
    if desc:
        ...
        p_sd.paragraph_format.space_before = Pt(3)
...
def add_media_entry(doc, title, org_date, desc, url=None, image_filename=None, img_width=Inches(3.4), east_asia="Microsoft JhengHei", url_label="🔗 Verified Source / DOI / URL: "):
...
# ========================================================
# 1. BUILD TRADITIONAL CHINESE RESUME (1.5x Font, No "我", Preface updated)
# ========================================================
def generate_zh():
    doc = create_base_doc()
    add_header(
    ...
    add_exp_item(
        doc,
        role="資訊部經理 / 數位轉型負責人",
        group_note="光男集團 / 峰晨集團 – 艾鉅電腦 (總部在台中，並在台北、深圳、上海、張家港設有據點 ) 上市櫃公司",
    ...
    print(f"Generated 1.5x ZH: {out_file}")
...
def generate_en():
    ...
    for d_title, bullets in comp_data:
        add_subheading(doc, d_title, size=SZ_H2)
        for b in bullets:
    ...
    add_exp_item(
        ...
        group_note="Kuang Nan Group / Fengchen Group – Aiju Computer (Headquartered in Taichung with offices in Taipei, Shenzhen, Shanghai, Zhangjiagang ) Publicly Listed Company",
    ...
    for d, s, desc in edu_list:
        ...
        p.paragraph_format.keep_with_next = True
        r = p.add_run(s)
        set_run_font(r, size=SZ_META, bold=True, color=COLOR_SECONDARY)
        add_body_p(doc, desc, space_after=4)

    add_heading_1(doc, "Awards, Keynotes, Publications & Speaking")
    add_subheading(doc, "1. Keynote Addresses & Professional Speaking", size=SZ_H2)
    ...
    add_subheading(doc, "4. Awards & Community Leadership", size=SZ_H2)
    ...
    doc.save(out_file)
    print(f"Generated 1.5x EN: {out_file}")
...
def generate_ja():
    ...
    for d_title, bullets in comp_data_ja:
        add_subheading(doc, d_title, size=SZ_H2, east_asia="Meiryo")
        for b in bullets:
    ...
    for d, s, desc in edu_list_ja:
        ...
        p.paragraph_format.keep_with_next = True
        r = p.add_run(s)
        set_run_font(r, size=SZ_META, bold=True, color=COLOR_SECONDARY, east_asia="Meiryo")
        add_body_p(doc, desc, space_after=4, east_asia="Meiryo")

    add_heading_1(doc, "受賞歴・基調講演・学術論文・メディア掲載・CSR", east_asia="Meiryo")
    add_subheading(doc, "1. 基調講演・講義実績 (Keynotes & Speaking)", size=SZ_H2, east_asia="Meiryo")
    ...
    add_subheading(doc, "4. 受賞歴 & 企業の社会的責任 (CSR)", size=SZ_H2, east_asia="Meiryo")
    ...
    doc.save(out_file)
    print(f"Generated 1.5x JA: {out_file}")
