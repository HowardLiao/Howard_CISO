...
# Standard Professional C-Level Resume Font Sizes
SZ_TITLE = Pt(20)
SZ_H1 = Pt(13)
SZ_H2 = Pt(11)
SZ_H3 = Pt(10.5)
...
def add_subheading(doc, text, space_before=8, space_after=2, color=COLOR_SECONDARY, size=SZ_H2, east_asia="Microsoft JhengHei"):
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(space_before)
    p.paragraph_format.space_after = Pt(space_after)
    p.paragraph_format.keep_with_next = True
...
# ========================================================
# 1. BUILD TRADITIONAL CHINESE RESUME (Standard Professional Typography)
# ========================================================
def generate_zh():
    doc = create_base_doc()
    add_header(
    ...
    add_exp_item(
        doc,
        role="資訊部經理 / 數位轉型負責人",
        group_note="光男集團 / 峰晨集團 – 艾鉅電腦 (總部在台中，並在台北、深圳、上海、張家港都設有據點) 上市櫃公司",
    ...
    print(f"Restored Standard ZH: {out_file}")

# Generate EN and JA standard
from build_all_resumes import build_en_docx, build_ja_docx

if __name__ == "__main__":
    generate_zh()
    build_en_docx()
    build_ja_docx()
    print("All 3 resumes successfully restored to standard executive Word format!")
